...
def show_main():
    ...
    with st.sidebar:
        ...
        if data_source == "手动输入":
            ...
            if st.button("▶️ 手动记录本次数据", type="primary", key="manual_btn"):
                ...
                st.session_state.history_data = pd.concat([st.session_state.history_data, pd.DataFrame([new_row])],
                                                          ignore_index=True)
                st.toast("✅ 手动数据已记录！")
        else:
    ...
    st.markdown("<p style='text-align: center; color: #64748B;'>基于当涂华水水务真实历史数据的智能分析系统</p>",
    ...
    with tab1:
        ...
        if st.session_state.history_data.empty:
            ...
            with st.container(border=True):
                ...
                st.info(
                    f"**测算说明**：假设水厂日处理规模 10 万吨，通过 AI 智能调控 SRT 和 F/M，源头减泥率约 {reduction_rate * 100:.1f}%，可显著降低污泥处置成本与碳排放。")
    ...
    with tab3:
        ...
        with col_m1:
            btn_linear = st.button("Linear", use_container_width=True, key="fi_btn_linear")
        with col_m2:
            btn_lasso = st.button("Lasso", use_container_width=True, key="fi_btn_lasso")
        with col_m3:
            btn_rf = st.button("RF", use_container_width=True, key="fi_btn_rf")
        with col_m4:
            btn_xgb = st.button("XGBoost", use_container_width=True, key="fi_btn_xgb")
        with col_m5:
            btn_all = st.button("📊 全部对比图", use_container_width=True, key="fi_btn_all")
        ...
        if btn_all:
            ...
            fig_all.update_layout(title=f"全部模型 - {target_var_fi} 特征重要性对比", barmode='group',
            ...
            st.dataframe(df_fi)
            st.download_button("📥 下载全部特征重要性数据表", df_fi.to_csv(index=False).encode('utf-8-sig'),
                               f"{target_var_fi}_全部特征重要性.csv", "text/csv", key="dl_fi_all")
        else:
            selected_model = None
            if btn_linear:
            ...
            if selected_model:
                if selected_model == "Linear":
                    importance = np.abs(current_models["Linear"].coef_)
                elif selected_model == "Lasso":
                    importance = np.abs(current_models["Lasso"].coef_)
                elif selected_model == "RF":
                    importance = current_models["RF"].feature_importances_
                else:
                    importance = current_models["XGBoost"].feature_importances_
                fig_fi = px.bar(x=importance, y=X.columns, orientation='h',
                                title=f"{selected_model} - {target_var_fi} 特征重要性")
                fig_fi.update_layout(template=st.session_state.theme)
                st.plotly_chart(fig_fi, use_container_width=True)
                df_single = pd.DataFrame({"特征": X.columns, "重要性": importance})
                st.dataframe(df_single)
                st.download_button(f"📥 下载 {selected_model} 特征重要性数据表",
                                   df_single.to_csv(index=False).encode('utf-8-sig'),
                                   f"{target_var_fi}_{selected_model}_特征重要性.csv", "text/csv", key="dl_fi_single")

    with tab4:
        st.subheader("🤖 模型性能评价与对比分析")
        target_var_metric = st.selectbox("选择目标变量进行评价", ["有机质占比", "污泥沉降指数SVI"],
                                         key="metric_target_var")
        ...
        with col_met1:
            btn_r2 = st.button("R²", use_container_width=True, key="metric_btn_r2")
        with col_met2:
            btn_rmse = st.button("RMSE", use_container_width=True, key="metric_btn_rmse")
        with col_met3:
            btn_mae = st.button("MAE", use_container_width=True, key="metric_btn_mae")
        with col_met4:
            btn_mape = st.button("MAPE", use_container_width=True, key="metric_btn_mape")
        with col_met5:
            btn_all_metrics = st.button("📊 全部评价指标对比", use_container_width=True, key="metric_btn_all")
        ...
        if btn_all_metrics:
            ...
            for metric_name in ["R²", "RMSE", "MAE", "MAPE"]:
                fig_all_metrics.add_trace(
                    go.Bar(x=model_names, y=[current_metrics[m][metric_name] for m in model_names], name=metric_name))
            fig_all_metrics.update_layout(title=f"全部评价指标对比 - {target_var_metric}", barmode='group',
                                          template=st.session_state.theme)
            ...
            st.info(
                f"**分析结论：** 综合 R²、RMSE、MAE、MAPE 四项指标来看，**{best_r2}** 模型在 R² 上表现最好，**{best_rmse}** 模型的 RMSE 最低。结合各项误差指标，推荐优先使用 **XGBoost**（或表现最优的模型）进行污泥减量化调控。这证明该模型在预测 {target_var_metric} 时具有极强的非线性拟合能力和泛化性能。")
        else:
            ...
            if selected_metric:
                ...
                fig_bar = px.bar(x=model_names, y=values, title=f"{selected_metric} 对比", color=model_names,
                                 template=st.session_state.theme)
                ...
                if selected_metric == "R²":
                    ...
                    st.info(
                        f"**分析结论：** 在 R²（决定系数）指标下，**{best_model}** 模型表现最优（R² = {max(values):.3f}）。R² 越接近 1，说明模型对目标变量（{target_var_metric}）的解释能力越强，预测效果越精准。")
                elif selected_metric in ["RMSE", "MAE", "MAPE"]:
                    best_model = model_names[np.argmin(values)]
                    st.info(
                        f"**分析结论：** 在 {selected_metric} 指标下，**{best_model}** 模型表现最优（{selected_metric} = {min(values):.3f}）。该指标越小，说明模型预测值与真实值的偏差越小，模型精度越高。")

        df_metrics = pd.DataFrame(current_metrics).T
        st.dataframe(df_metrics)
        st.download_button(f"📥 下载 {target_var_metric} 评价指标表", df_metrics.to_csv().encode('utf-8-sig'),
                           f"{target_var_metric}_评价指标.csv", "text/csv", key="dl_metrics")
        ...
        df_error = pd.DataFrame({
            "模型": ["Linear"] * len(y_test) + ["Lasso"] * len(y_test) + ["RF"] * len(y_test) + ["XGBoost"] * len(
                y_test),
            "绝对误差": np.concatenate([
        ...
        fig_box = px.box(df_error, x="模型", y="绝对误差", color="模型",
                         title=f"Absolute Error Distribution - {target_var_metric}", template=st.session_state.theme)
        st.plotly_chart(fig_box, use_container_width=True)

        st.markdown("### 📦 误差分布分析")
        best_box_model = df_error.groupby("模型")["绝对误差"].median().idxmin()
        st.info(
            f"**分析结论：** 从箱线图可以看出，**{best_box_model}** 模型的误差分布最集中，中位数最低，且极端离群值较少。这表明该模型在应对不同工况波动时，具有更强的稳定性和鲁棒性。")

    with tab5:
        st.subheader("🔍 SHAP 模型可解释性分析")
        target_var_shap = st.selectbox("🎯 请选择SHAP分析的目标变量", ["有机质占比", "污泥沉降指数SVI"],
        ...
        fig_bee.update_layout(title=f"SHAP Beeswarm Plot - {target_var_shap}", xaxis_title="SHAP Value",
                              yaxis_title="Feature", template=st.session_state.theme)
        ...
        fig_bar_shap = px.bar(x=mean_shap, y=X.columns, orientation='h',
                              title=f"SHAP Feature Importance - {target_var_shap}", template=st.session_state.theme)
        ...
        st.download_button(f"📥 下载 {target_var_shap} SHAP值分析表", df_shap.to_csv(index=False).encode('utf-8-sig'),
                           f"{target_var_shap}_SHAP分析表.csv", "text/csv", key="dl_shap")

    st.markdown("<br><p style='text-align: center; color: #64748B;'>© 2026 驰星队 · 马鞍山学院</p>",
                unsafe_allow_html=True)
